...
const Auth = {
  ...
  /**
   * Password recovery is disabled until ownership verification is implemented.
   * Never change credentials on the strength of an email address alone.
   */
  async resetPassword(email, newPassword) {
    return {
      ok: false,
      error: 'Password recovery is unavailable in this demo. A verified server-side reset flow is required.',
    };
  },
  ...
  guard(returnPath) {
    if (!this.isLoggedIn()) {
      ...
      loginUrl.searchParams.set('next', returnPath || './dashboard.html');
